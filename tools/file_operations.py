...
class ShellFileOperations(LintMixin, SearchMixin, FileOperations):
    ...
    def __init__(self, terminal_env, cwd: str = None, *, authoritative_cwd: bool = False):
        self.env = terminal_env
        # Never os.getcwd(): that is the HOST path, absent inside container backends.
        self.cwd = cwd or getattr(terminal_env, 'cwd', None) or \
                   getattr(getattr(terminal_env, 'config', None), 'cwd', None) or "/"
        self._authoritative_cwd = authoritative_cwd
    ...
    def _exec(self, command: str, cwd: str = None, timeout: int = None,
        ...
        effective_cwd = cwd or (
            self.cwd if self._authoritative_cwd else getattr(self.env, 'cwd', None) or self.cwd
        )
