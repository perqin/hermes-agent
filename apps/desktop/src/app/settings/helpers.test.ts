...
describe('settings helpers', () => {
  ...
  it('does not shadow discovery-driven terminal backend options', () => {
    expect(enumOptionsFor('terminal.backend', 'configured_box', {})).toBeUndefined()
  })
  ...
  describe('sectionFieldEntries', () => {
    it('shows config fields declared by the selected terminal backend', () => {
      const schema = {
        'terminal.backend': { type: 'select' as const, options: ['local', 'configured_box'] },
        'terminal.backends.configured_box.workspace': {
          type: 'string' as const,
          terminal_backend: 'configured_box'
        },
        'terminal.backends.other.workspace': {
          type: 'string' as const,
          terminal_backend: 'other'
        }
      }
      ...
      expect(advancedKeys).toContain('terminal.backends.configured_box.workspace')
      expect(advancedKeys).not.toContain('terminal.backends.other.workspace')
    })
